...
public final class AtomixRoutePolicyTest {
    ...
    private final List<Address> addresses = Arrays.asList(
        Address.from("127.0.0.1", AvailablePortFinder.getNextAvailable()),
        Address.from("127.0.0.1", AvailablePortFinder.getNextAvailable()),
        Address.from("127.0.0.1", AvailablePortFinder.getNextAvailable())
    );
    ...
    private void run(Address address) {
        try {
            ...
            service.setClusterId("camel");
            service.setId("node-" + address.port());
            service.setMemberAddress(address.host(), address.port());
            service.setMulticastEnabled(true);
            service.setProfiles(Profile.dataGrid());
            ...
            context.addRoutes(new RouteBuilder() {
                ...
                public void configure() throws Exception {
                    from("timer:atomix3?delay=1s&period=1s")
